...
def main():
    ...
    print("1. Fetch data from Garmin Connect using the API.")
    print("2. Use existing 'processed_strength_activities.json' file.")
    choice = input("Enter 1 or 2: ")

    if choice == '1':
        # Import main_garmin_script and call its main() function
        try:
            import main_garmin_script
        except ImportError as e:
            print(f"Error importing 'main_garmin_script.py': {e}")
            sys.exit(1)
        print("Fetching data from Garmin Connect...")
        main_garmin_script.main()
        print("Data fetched and processed.")
        
        # Check if the processed data file was created
        if not os.path.exists(processed_data_file):
            print(f"Error: '{processed_data_file}' not found after processing.")
            sys.exit(1)
    elif choice == '2':
        if not os.path.exists(processed_data_file):
            print(f"Error: '{processed_data_file}' not found.")
            sys.exit(1)
        print("Using existing 'processed_strength_activities.json' file.")
    else:
    ...
    # Plot the muscle map using the Front view by default
    plot_muscle_map(
