...
def update_chart(selected_metric, start_date, end_date, n_clicks, goal_value):
    ...
    fig.update_layout(
        ...
        xaxis=dict(
            ...
            dtick="D1",           # Daily tick marks for each date
            tickangle=45,         # Rotate tick labels for better readability
            showgrid=True,        # Show vertical grid lines for alignment
            gridcolor="lightgrey"  # Subtle grid color for clarity
        ),
        yaxis=dict(
        ...
        # Optionally, add background color to every other day for further visual alignment
    fig.update_xaxes(
        tickformat="%b %d",
        tickangle=45,
        showgrid=True,
